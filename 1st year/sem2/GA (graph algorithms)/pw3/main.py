...
def prim_algo(g: DirectedGraph, s):
    """
    This function finds the minimum spanning tree of a graph
    ...
    v_new.append(s)
    min = 9999
    while len(v_new) < num_v:
        for v in v_new:
            for edge in g.parse_outbound_edges(v):
                if edge[1] not in v_new and edge[2] < min:
                    min = edge[2]
                    min_edge = edge
...
if __name__ == "__main__":
    ...
    backwards_dijkstra(graph, 100, 1)
