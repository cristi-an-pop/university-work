...
void testRemoveBetween() {
	...
	ListIterator it = getIterator(0, list);
	ListIterator it2 = getIterator(98, list);

	list.removeBetween(it, it2);
	assert(list.size() == 1);
	assert(list.first().getCurrent() == 99);
	testIteratorSteps(list);

	for (int i = 0; i < 100; i++) {
		list.addToEnd(i);
	}
	ListIterator it3 = getIterator(0, list);
	ListIterator it4 = getIterator(50, list);

	list.removeBetween(it3, it4);
	assert(list.size() == 50);
	assert(list.first().getCurrent() == 50);
	testIteratorSteps(list);
}
