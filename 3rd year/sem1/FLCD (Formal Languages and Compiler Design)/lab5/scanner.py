...
class Scanner:
    def __init__(self, token_file, input_program_file, fa_file):
        ...
        self.finite_automata.load_from_file(fa_file)
    ...
    def classify_token(self, token):
        if token in self.reserved_words:
            ...
            return "separator", 0
        elif self.finite_automata.accepts(token, start_state="q0", final_states={"q1"}):
            return "identifier", self.symbol_table.add(token)
        elif self.finite_automata.accepts(token, start_state="q0", final_states={"q2"}):
...
if __name__ == '__main__':
    scanner = Scanner("token.in.txt", ".//input_data//p1.txt", "FA2.in")
    scanner.analyze()
    print("Complete")
