        ...
        while self.state not in {"f", "e"}:
            ...
            if self.state == "q":
                if not self.input_stack:
                    ...
                    print("==========================")
                    print(f"Checking position: {self.index}, Expected match length: {terminal_length}")
                    print(f"Input string length: {len(input_string)}")
                    print(f"Substring to match: '{input_string[self.index:self.index + terminal_length]}'")
                    print(f"Terminal to match: '{terminal}'")

                    # Check if the terminal matches the substring of the input string
                    if self.index + terminal_length <= len(input_string):
                        substring = input_string[self.index:self.index + terminal_length]
                        print(f"Comparing substring '{substring}' with terminal '{terminal}'")

                        if substring == terminal:
                            # Terminal matched, advance the index
                            self.advance(terminal_length)  # Pass the length of the terminal to advance
                        else:
                            # If the match fails, it's a momentary insuccess
                            print("Terminal mismatch, backtracking...")
                            self.momentary_insuccess()
                    else:
                        # If we reach the end of the input string and cannot match the terminal, backtrack
                        print(f"Index {self.index} is out of bounds for terminal match.")
                        self.momentary_insuccess()
